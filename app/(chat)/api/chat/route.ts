  ...
  try {
    ...
    if (providers.length === 0) {
      return new ChatbotError(
        "bad_request:api",
        "No API keys configured. Please add a provider key in settings."
      ).toResponse();
    }
    ...
    const capabilities = getCapabilitiesFromModels(allModels);
    const modelCapabilities = capabilities[chatModel];
    const isReasoningModel = modelCapabilities?.reasoning === true;
    const supportsTools = modelCapabilities?.tools === true;
